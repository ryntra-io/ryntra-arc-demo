...
test("the file adapter fails closed on corrupt lifecycle state", async () => {
  const directory = temporaryDirectory();
  try {
    writeFileSync(join(directory, "transactionIndex.json"), "{not-json", "utf8");
    const store = createFileGuardStore({ directory });
    await assert.rejects(
      () => store.collection("transactionIndex").has("eip155:5042002:0xabc"),
...
test("overlapping batch claims are all-or-none with no loser residue", async () => {
  const directory = temporaryDirectory();
  try {
    for (const [indexNumber, subject] of [
      createMemoryGuardStore(),
      createFileGuardStore({ directory }),
    ].entries()) {
      const index = subject.collection("transactionIndex");
      const txKey = `eip155:5042002:0xbatch${indexNumber}`;
      const batches = [
        [
          { key: txKey, value: "tenant-a:int_a" },
          { key: "intent:tenant-a:int_a", value: txKey },
        ],
        [
          { key: txKey, value: "tenant-b:int_b" },
          { key: "intent:tenant-b:int_b", value: txKey },
        ],
      ];
...
test("store selection never falls back to memory when a durable store was requested", () => {
  ...
  assert.throws(
    () => resolveGuardStore({ RYNTRA_GUARD_STORE: "memroy" }),
    /Unsupported RYNTRA_GUARD_STORE/,
  );
...
test("deployment shape defaults to the safe assumption", () => {
  ...
  assert.throws(
    () => resolveGuardDeployment({ RYNTRA_GUARD_DEPLOYMENT: "multiple" }),
    /Unsupported RYNTRA_GUARD_DEPLOYMENT/,
  );
  assert.equal(
...
test("failed file persistence never becomes successful in the warm in-memory snapshot", async () => {
  const directory = temporaryDirectory();
  try {
    const store = createFileGuardStore({ directory });
    const intents = store.collection("intents");
    await intents.set("tenant-a:int_existing", { id: "int_existing" });

    /* The adapter writes through this exact temporary path. Making it a
       directory deterministically forces every write to fail without relying
       on platform-specific permissions. */
    mkdirSync(join(directory, "intents.json.tmp"));

    await assert.rejects(() => intents.set("tenant-a:int_set", { id: "int_set" }));
    await assert.rejects(() =>
      intents.insertIfAbsent("tenant-a:int_insert", { id: "int_insert" }),
    );
    await assert.rejects(() =>
      intents.insertAllIfAbsent([
        { key: "tenant-a:int_batch_a", value: { id: "int_batch_a" } },
        { key: "tenant-a:int_batch_b", value: { id: "int_batch_b" } },
      ]),
    );
    await assert.rejects(() => intents.delete("tenant-a:int_existing"));

    assert.deepEqual(await intents.get("tenant-a:int_existing"), { id: "int_existing" });
    for (const key of ["int_set", "int_insert", "int_batch_a", "int_batch_b"]) {
      assert.equal(await intents.get(`tenant-a:${key}`), undefined, key);
    }

    const reopened = createFileGuardStore({ directory });
    const cold = reopened.collection("intents");
    assert.deepEqual(await cold.get("tenant-a:int_existing"), { id: "int_existing" });
    assert.equal(await cold.get("tenant-a:int_set"), undefined);
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
});
