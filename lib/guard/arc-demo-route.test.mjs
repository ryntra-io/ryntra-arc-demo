...
test("GET exposes only truthful testnet capability state and creates an httpOnly session", async () => {
  ...
  assert.equal(
    body.data.authorizationMethod,
    "PARTNER_AUTHENTICATED_ON_AUTHENTICATED_V1_ONLY",
  );
  assert.ok(
    body.data.limitations.includes("ANONYMOUS_DEMO_CANNOT_AUTHORIZE_OR_RECORD_EXECUTION"),
  );
  ...
  assert.equal(body.data.capabilityStatus, arcStatus.label);
  assert.equal(body.data.liveExecutionVerified, arcStatus.isCurrent);
  assert.equal(
    body.data.limitations.includes("LIVE_EXECUTION_NOT_VERIFIED"),
    false,
  );
  assert.equal(
    body.data.limitations.includes("RECORDED_RUN_REQUIRES_REVERIFICATION"),
    !arcStatus.isCurrent,
  );
...
test("omitting or rotating the demo session cannot reset the anonymous request quota", async () => {
  const { POST } = await loadRoute();
  try {
    for (let index = 0; index < 120; index += 1) {
      const cookie = index % 2 === 0
        ? {}
        : { cookie: `ryntra_arc_demo_session=demo_${index.toString(16).padStart(32, "0")}` };
    ...
    const blocked = await POST(
      new Request("http://localhost/api/arc-guard", {
        ...
        headers: {
          "content-type": "application/json",
          cookie: "ryntra_arc_demo_session=demo_ffffffffffffffffffffffffffffffff",
        },
        body: JSON.stringify({}),
      }),
    );
...
test("the anonymous demo cannot create partner authorization", async () => {
  const { POST } = await loadRoute();
  const response = await POST(
    new Request("http://localhost/api/arc-guard", {
      method: "POST",
  ...
  assert.equal(response.status, 503);
  assert.equal(body.error.code, "CAPABILITY_UNAVAILABLE");
  assert.equal(body.error.requiredAction, "USE_AUTHENTICATED_PARTNER_API");
});
