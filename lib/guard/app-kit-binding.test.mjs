...
const policy = {
  ...
  immutable: true,
  rules: [
    { id: "chain", type: "ALLOWED_CHAIN", value: "eip155:5042002", onViolation: "BLOCK" },
    { id: "pair", type: "ALLOWED_PAIR", value: [USDC, EURC], onViolation: "BLOCK" },
    { id: "debit", type: "MAX_TOTAL_DEBIT", value: "100.00", currencyAssetRef: USDC, onViolation: "BLOCK" },
    { id: "age", type: "MAX_QUOTE_AGE_SECONDS", value: 120, onViolation: "INSUFFICIENT_EVIDENCE" },
    { id: "slippage", type: "MAX_SLIPPAGE_BPS", value: "25", onViolation: "REVIEW" },
    { id: "human", type: "HUMAN_AUTHORIZATION_REQUIRED", value: true, onViolation: "REQUIRE_AUTHORIZATION" },
  ],
};
...
test("authorization binds the App Kit request hash but unbound execution stays unavailable", async () => {
  ...
  assert.equal(
    await service.store.collection("executions").has(`tenant_demo:${intent.id}`),
    false,
  );
  assert.equal(
    await service.store.collection("transactionIndex").has(`${fingerprint.chainRef}:${transactionHash}`),
    false,
  );
});
