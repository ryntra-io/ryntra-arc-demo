...
test("five Guard status axes preserve exact vocabulary and canonical lifecycle precedence", async () => {
  const {
    GUARD_STATUS_AXIS_DEFINITIONS,
    resolveGuardStatusAxes,
  } = await import("./status-values.ts");

  assert.deepEqual(GUARD_STATUS_AXIS_DEFINITIONS, [
  ...
  assert.deepEqual(resolveGuardStatusAxes(evaluation), evaluation);
  assert.deepEqual(resolveGuardStatusAxes(evaluation, receipt), receipt);
  assert.deepEqual(resolveGuardStatusAxes(null, null), {
    evidenceStatus: null,
    policyDecision: null,
...
test("demo client carries the required public prototype labels and truthful receipt state", async () => {
  ...
  /* The rail is no longer typed in this file, so asserting its literals here
     would only re-bless a hard-coded string — which is what let it drift in the
     first place. Assert the labels where they now live, and assert that the
     component binds to them. */
  const { ARC_PROJECT } = await import("../../app/arc/arc-project.ts");
  assert.deepEqual(
    [...ARC_PROJECT.claimRail],
    ["INDEPENDENT PROJECT", "TESTNET ONLY", "NOT AUDITED", "NOT FINANCIAL ADVICE"],
  );
  assert.match(source, /ARC_PROJECT\.claimRail\.map/);
  assert.match(source, /ARC_PROJECT\.network\.toUpperCase\(\)/);
  assert.doesNotMatch(source, /<span>INDEPENDENT PROJECT<\/span>/);

  for (const label of ["NOT VERIFIED", "Not finalized"]) {
    assert.match(source, new RegExp(label));
  }

  /* The claim rail remains static project scope, while the strongest capability
     label comes from the no-store API and therefore follows registry freshness. */
  assert.match(source, /import \{ ARC_PROJECT, RECORDED_RUN \} from "\.\.\/arc\/arc-project\.ts"/);
  assert.match(source, /capability\?\.capabilityStatus/);
  assert.match(source, /CHECKING CAPABILITY FRESHNESS/);
  assert.doesNotMatch(source, /ARC_PROJECT\.status/);
  ...
  assert.match(source, /reconciliationStatus/);
  assert.match(source, /GUARD_STATUS_AXIS_DEFINITIONS\.map/);
  assert.match(source, /statusAxes\[axis\.key\]/);
  assert.match(source, /GUARD_SDK_EXAMPLE as SDK_EXAMPLE/);
  assert.match(source, /GUARD_API_EXAMPLE as API_EXAMPLE/);
  assert.doesNotMatch(source, /executionFingerprint|\btxHash\b/);
  assert.doesNotMatch(source, /NOT COLLECTED|NOT EVALUATED|NOT STARTED|WAITING FOR EVIDENCE/);
  assert.match(source, /const statusAxes = resolveGuardStatusAxes\(evaluation, receipt\)/);
  assert.match(source, /memoSupported: false/);
  assert.match(source, /Authenticated \/v1 authorization required/);
  assert.match(source, /const canAuthorize = false/);
  ...
  assert.match(routeSource, /ANONYMOUS_MONEY_ACTIONS/);
  assert.match(routeSource, /USE_AUTHENTICATED_PARTNER_API/);
  assert.match(routeSource, /ANONYMOUS_DEMO_AUTHORIZATION_UNAVAILABLE/);
});
