...
test("provider-neutral evidence fields are explicit and unavailable coverage cannot allow policy", async () => {
  ...
  const policy = {
    rules: [
      ...
      },
      {
        ...
        value: "100.00",
        currencyAssetRef: evidence.facts.sellAssetRef,
      },
  ...
  const intent = {
    ...
    amount: evidence.facts.amountIn,
    leverage: null,
    recipient: WALLET,
    venueRef: evidence.facts.venueRef,
    routeRef: evidence.facts.routeRef,
...
test("Arc native and ERC-20 USDC units stay distinct and fallback binds exact EOA transfer calldata", async () => {
  ...
  const prepared = await prepareArcUsdcTreasuryTransfer({
    ...
    collectOnchainState: async () => ({
      ...
      nativeBalanceBaseUnits: "1000066000000000000",
      gasLimit: "65000",
  ...
  const confirmed = await reconcileArcUsdcTreasuryTransfer({
    ...
    collectTransactionState: async () => ({
      ...
      blockTimestamp: "2026-08-06T12:00:01.000Z",
    }),
  });
  assert.equal(confirmed.observedState, "CONFIRMED");
  assert.equal(confirmed.observedAt, "2026-08-06T12:00:01.000Z");
...
test("Arc transfer preflight reserves one underlying USDC balance for amount plus gas", async () => {
  const { prepareArcUsdcTreasuryTransfer } = await import("./arc-usdc-transfer.ts");
  const request = {
    walletType: "EOA",
    walletAddress: WALLET,
    recipientAddress: RECIPIENT,
  ...
  await assert.rejects(
    () =>
      prepareArcUsdcTreasuryTransfer({
        ...
        collectOnchainState: async () => ({
          ...baseState,
          /* Enough for the ERC-20 amount and for gas independently, but not
             for both views of the same underlying Arc USDC balance. */
          nativeBalanceBaseUnits: "1000001000000000000",
        }),
      }),
    (error) => error?.code === "ARC_TRANSFER_INSUFFICIENT_BALANCE",
  );

  const exactBoundary = await prepareArcUsdcTreasuryTransfer({
    request,
    collectOnchainState: async () => ({
      ...baseState,
      nativeBalanceBaseUnits: "1000066000000000000",
  ...
  assert.equal(exactBoundary.evidence.facts.totalDebit, "1.000066");
  assert.equal(exactBoundary.transaction.value, "0x0");
});

test("Arc transfer failures never disclose the configured RPC endpoint", async () => {
  const { prepareArcUsdcTreasuryTransfer } = await import("./arc-usdc-transfer.ts");
  let captured;
  try {
    await prepareArcUsdcTreasuryTransfer({
      request: {
        walletType: "EOA",
        walletAddress: WALLET,
        recipientAddress: RECIPIENT,
      ...
      collectOnchainState: async () => {
        throw new Error("https://rpc.example/SECRET_SENTINEL?api_key=SECRET_SENTINEL");
      },
    });
  } catch (error) {
    captured = error;
  }
  assert.equal(captured?.code, "ARC_TRANSFER_EVIDENCE_UNAVAILABLE");
  assert.match(captured?.message ?? "", /configured endpoint/i);
  assert.doesNotMatch(captured?.message ?? "", /SECRET_SENTINEL|rpc\.example/);
});
